...
for c in cat.keys():
  for t in ["fav", "comment", "view", "mylist"]:
    ...
    if not os.path.exists("./legacy/total/{}".format(t)):
        os.mkdir("./legacy/total/{}".format(t))
    batCount = 0
    while True:
      res = session.get(
        "https://nvapi.nicovideo.jp/v1/ranking/legacy?date=2019-06-26&type={}&categoryKey={}&span=total".format(t, c),
        headers=HEADERS
      );
      r = res.json()
      if len(r["data"]["items"]) <= 0:
        break
      with open("./legacy/total/{}/{}.json".format(t,c), mode="w", encoding="utf-8") as f:
        f.write(res.text)
        print("Save:{}/{}".format(c,t))
      break
